const mongoose = require('mongoose');
const User = require('../models/User');
const readline = require('readline');

const rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout
});

// Connect to MongoDB
mongoose.connect('mongodb://localhost:27017/medicalEducation', {})
  .then(() => {
    console.log('Connected to MongoDB');
    deleteAdmin();
  })
  .catch(err => {
    console.error('MongoDB connection error:', err);
    process.exit(1);
  });

async function deleteAdmin() {
  try {
    // First list all admin users to choose from
    const admins = await User.find({ role: 'admin' }).select('-password');
    
    if (admins.length === 0) {
      console.log('No admin users found in the database.');
      process.exit(0);
    }
    
    console.log('\n===== ADMIN USERS =====');
    admins.forEach((admin, index) => {
      console.log(`[${index + 1}] ${admin.username} (${admin.email})`);
    });
    
    // Ask which admin to delete
    const adminIndexStr = await question('\nEnter the number of the admin user to delete: ');
    const adminIndex = parseInt(adminIndexStr) - 1;
    
    if (isNaN(adminIndex) || adminIndex < 0 || adminIndex >= admins.length) {
      console.log('Invalid selection.');
      process.exit(1);
    }
    
    const selectedAdmin = admins[adminIndex];
    
    // Confirm deletion
    const confirmation = await question(`\nAre you sure you want to delete admin: ${selectedAdmin.username} (${selectedAdmin.email})? (yes/no): `);
    
    if (confirmation.toLowerCase() !== 'yes') {
      console.log('Operation cancelled.');
      process.exit(0);
    }
    
    // Delete the admin
    await User.findByIdAndDelete(selectedAdmin._id);
    
    console.log(`\nAdmin user ${selectedAdmin.username} has been successfully deleted.`);
    
    // Disconnect from MongoDB
    await mongoose.disconnect();
    console.log('\nDisconnected from MongoDB');
    process.exit(0);
    
  } catch (error) {
    console.error('Error deleting admin user:', error);
    process.exit(1);
  }
}

// Helper function to prompt for input
function question(prompt) {
  return new Promise(resolve => {
    rl.question(prompt, answer => {
      resolve(answer);
    });
  });
} 